...
- Скан PDF без текстового слоя (книга FineReader, 214 стр. картинок) → джоба `error`. OCR не делаем. Пересмотрено ниже, «Решения OCR сканов».
...
- Модель: `deepseek-community/DeepSeek-OCR-2`, нативный класс `transformers` 5.x. Оригинальный `deepseek-ai/DeepSeek-OCR` требует `trust_remote_code` и `transformers==4.46.3` — откат сломал бы Flux и bge-m3. Лишних зависимостей (`einops`, `addict`, `easydict`, flash-attn) не нужно.
- VLM через Ollama не берём: специализированная OCR-модель точнее на книжных сканах.
- Отдельный тенант `ocr`: torch-модель ~3B bf16 не делит VRAM ни с Ollama, ни с Flux. Lock держится на всю книгу, страницы по одной.
- OCR только при пустом текстовом слое. Результат — `source.txt` рядом с `source.pdf`; повторная индексация берёт его, повторный OCR не нужен.
- Размер страницы задаёт `OCR_DPI` + `OCR_MAX_PATCHES` (у нативного процессора нет `base_size` / `image_size` / `crop_mode`).
